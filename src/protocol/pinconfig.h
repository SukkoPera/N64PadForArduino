...
#define N64_CUSTOM_INTMAN
	...
	#define PREPARE_INTERRUPT() {MCUCR |= (1 << ISC01); MCUCR &= ~(1 << ISC00);}
	#define ENABLE_INTERRUPT() {GIFR |= (1 << INTF0); GIMSK |= (1 << INT0);}
	#define DISABLE_INTERRUPT() {GIMSK &= ~(1 << INT0);}
	...
	//~ #define N64PAD_USE_INTX
	//~ #define N64PAD_INT_VECTOR INT0_vect
	//~ #define PREPARE_INTERRUPT() {EICRA |= (1 << ISC01); EICRA &= ~(1 << ISC00);}
	//~ #define ENABLE_INTERRUPT() {EIFR |= (1 << INTF0); EIMSK |= (1 << INT0);}
	//~ #define DISABLE_INTERRUPT() {EIMSK &= ~(1 << INT0);}
	...
	#define PREPARE_INTERRUPT() {EICRA |= (1 << ISC11); EICRA &= ~(1 << ISC10);}
	#define ENABLE_INTERRUPT() {EIFR |= (1 << INTF1); EIMSK |= (1 << INT1);}
	#define DISABLE_INTERRUPT() {EIMSK &= ~(1 << INT1);}
	...
	//~ #define PREPARE_INTERRUPT() {PCMSK0 |= (1 << PCINT4);}
	//~ #define ENABLE_INTERRUPT() {PCIFR |= (1 << PCIF0); PCICR |= (1 << PCIE0);}
	//~ #define DISABLE_INTERRUPT() {PCICR &= ~(1 << PCIE0);}
	...
	#define PREPARE_INTERRUPT() {EICRB |= (1 << ISC51); EICRB &= ~(1 << ISC50);}
	#define ENABLE_INTERRUPT() {EIFR |= (1 << INTF5); EIMSK |= (1 << INT5);}
	#define DISABLE_INTERRUPT() {EIMSK &= ~(1 << INT5);}
#else
